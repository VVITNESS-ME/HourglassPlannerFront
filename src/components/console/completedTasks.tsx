...
const CompletedTasks: React.FC = () => {
  const tasks = [
    ...
    { text: '굉장히 길고 긴 추가 완료 333333333333333', color: 'yellow' },
  ...
  return (
    <CardLayout title="해낸 일"  width="w-80" height="h-72">
      <ul>
        {tasks.map((task, index) => (
          <li key={index} className="flex justify-between items-center mb-2 whitespace-nowrap">  {/* 오른쪽 패딩 추가 */}
            <span>{task.text}</span>
